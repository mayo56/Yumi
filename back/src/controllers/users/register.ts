...
export default {
    ...
    exec(request, args) {
        ...
        db().then(async (database) => {
            ...
            const email_exists = await database.all("SELECT * FROM users WHERE email = ? ;", [body.email]);
            ...
            const pseudo_exists = await database.all("SELECT * FROM users WHERE pseudo = ? ;", [body.pseudo]);
